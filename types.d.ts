declare global {
    ...
    /**
     * Тип предмета / занятия
     * table: subject_type
     */
    interface SubjectType {
        id: number;
        name: LessonType | string;
    }

    /**
     * Количество занятий определённого типа у предмета
     * table: subject_type_amount
     */
    interface SubjectTypeAmount {
        subject_id: number;
    ...
    /**
     * Связь назначения и учебных групп
     * table: assignment_study_groups
     */
    ...
    /**
     * Связь назначения и предметов
     * table: assignment_subjects
     */
    ...
    interface Schedule {
        id: number;
        created_at: Date;
        schedule_variant: string; // CLOB (JSON / text)
        study_unit_id: number;
    }
    ...
    interface ComplexCondition {
        ...
        lesson_connections?: {
            ...
            /**
             * Участники связи.
             * Здесь предлагаю сделать вот такой объект вместо закрепления полей.
             * Таким образом можно будет дополнять различные связи, если понадобяться еще.
             * Этот объект и поля в нем соотвтественно зависят от поля type выше.
             */
            connection_subjects: Record<string, any>;
        }[];
